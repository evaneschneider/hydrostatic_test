...
istart = 200 
iend = 300 
dnamein='/Volumes/data1/evan/data/disk_3D/128/hdf5/'
dnameout='./128/png/'
...
for i in range(istart,iend+1):
  ...
  rho_pd_min = 5.9814
  rho_pd_max = 8.4796
  #print(rho_pd_min, rho_pd_max)

  #set color scale 
  cmin=100/360.
  cmax=300/360.
  cpd = (np.clip(log_pdz, rho_pd_min, rho_pd_max) - rho_pd_min + 0.01) / (rho_pd_max-rho_pd_min)
  ...
  rho_pd_min =  5.8935
  rho_pd_max = 9.5594  
  cpd = (np.clip(log_pdy, rho_pd_min, rho_pd_max) - rho_pd_min + 0.01) / (rho_pd_max-rho_pd_min)
  H = (cmin-cmax)*cpd + cmax
  H = H.T
  ...
  a0.autoscale(False)
  a0.text(360, 560, str(int(t/1000))+' Myr', color='white', horizontalalignment='right')
...
"""
  for zi in range(nz):
    ...
    plt.savefig(dnameout+'v_'+str(i)+str(zi)+'.png', dpi=300)
    plt.close(fig)
"""
