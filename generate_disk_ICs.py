...
sigma_e = 0.25*M_d*np.exp(-r/R_g) / (2*np.pi*R_g*R_g) # exact solution for comparison
...
# solve the integral over z for each r
...
# central density comes from the surface density distribution
rho_0 = 0.5 * sigma / sol
...
# bin the mass every 4 cells in z to ensure mass conservation
v_cell = dx*dy*dz
M = d_temp*v_cell
dz_new = 4*dz
...
"""
# old version of setting densities (without mass conservation)
d = np.empty([nx,ny,2*nz])
for kk in range(0, nz):
  d[:,:,nz+kk] = d[:,:,nz-kk-1] = rho_0*np.exp(B_h*(A_h - phi_halo(z[kk],r_pos,R_h)) + B_d*(A_d - phi_disk(z[kk],r_pos,R_d,Z_d)))
# now redefine everything in terms of the new z array
nz = int(2*nz)
z = np.linspace(-zmax+0.5*dz,zmax-0.5*dz,nz)  
"""
...
# set the rest of the disk properties
...
for kk in range(0, nz):
  for jj in range(0, ny):
    for ii in range(0, nx):
      ...
      a_d = gr_disk(zp, rp, G, M_d, R_d, Z_d) # centripetal acceleration due to disk potential
      a_h = 0.0 # haven't calculated this yet
      a = a_d + a_h
      #circular velocity from radial acceleration
      v = np.sqrt(rp*a)
      vr[ii,jj,kk] = v
      vx = -np.sin(phi)*v
      vy = np.cos(phi)*v
      ...
      my[ii,jj,kk] = dp*vy
      mz[ii,jj,kk] = dp*vz
      # pressure assuming isothermal
      P = dp*(cs**2)/(gamma)
      E[ii,jj,kk] = P/(gamma-1.0) + 0.5*dp*(vx*vx + vy*vy)

# write the array to an hdf5 file
#dname='/Volumes/data1/evan/data/disk_3D/128'
dname='.'

dims = [nx,ny,nz]
f = h5py.File(dname+'/hdf5/0.h5', 'w')
f.attrs.create("gamma", gamma)
f.attrs.create("t", 0.0)
f.attrs.create("n_step", 0)
f.attrs.create("dims", dims)
...
# to get a single column for a 1D test, use this code
# note that you'll have to set the correct value of r_disk
# in the 'calc_g_1D' function in hydro_cuda.cu
"""
xi,yi = 32,32
print(r_pos[xi,yi])
dims = [nz,1,1]
...
f.attrs.create("n_step", 0)
f.attrs.create("dims", dims)
f.create_dataset('density', data=d[xi,yi,:])
f.create_dataset('momentum_x', data=mx[xi,yi,:])
f.create_dataset('momentum_y', data=my[xi,yi,:])
f.create_dataset('momentum_z', data=mz[xi,yi,:])
f.create_dataset('Energy', data=E[xi,yi,:])
f.close()
"""
# make a plot of the density projections
